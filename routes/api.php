...
Route::middleware('web')->middleware('auth:sanctum')->get('/user', function (Request $request) {
    return $request->user();
});
...
Route::middleware('web')->post('/login', [AuthenticatedSessionController::class, 'store'])
    ->name('login');
Route::post('/user/update', [AuthenticatedSessionController::class, 'updateUser'])->middleware('auth:sanctum');
